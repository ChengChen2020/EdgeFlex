...
class HPPO:
    def __init__(self, num_users, num_states, num_channels, lr_a, lr_c, gamma, lam, repeat_time, batch_size,
                 eps_clip, w_entropy):
        self.actor = Actor(num_states, 3, num_parts=4, num_embeds=3).cuda()
        ...
        self.buffer = PPOBuffer(num_users)
    ...
    def select_action(self, state, test=False):
        with torch.no_grad():
            ...
            prob_points, prob_parts, prob_embeds = self.actor(state)
            ...
            if not test:
                ...
                self.buffer.actor_buffer.actions_channel.append(part)
                self.buffer.actor_buffer.actions_power.append(embed)

                self.buffer.actor_buffer.logprobs_point.append(dist_point.log_prob(point))
                self.buffer.actor_buffer.logprobs_channel.append(dist_channel.log_prob(channel))
